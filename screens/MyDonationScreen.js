...
componentDidMount(){
  ...
  getDonorDetails=(donorId)=>{
    db.collection('Users').where('email','==',donorId)
    .onSnapshot(
      snapshot=>{
        snapshot.forEach(doc=>{
          this.setState({donorName:doc.data().username})
        })
      }
    )
  }
  ...
  getallDonations(){
   ...
   sendBook=(bookDetails)=>{
     if(bookDetails.request_status === "Book Sent"){
       ...
       var requestStatus2 = "Book Sent"
       db.collection("all_donations").doc(bookDetails.doc_id).update({
         "request_status" : "Book Sent"
       })
       this.sendNotification(bookDetails,requestStatus2)
     }
   }
